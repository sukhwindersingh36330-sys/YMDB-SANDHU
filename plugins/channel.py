    ...
    if not movie_doc:
        ...
        if TMDB_POSTER:
            ...
            if not details or not details.get("poster_url"):
                logger.info("TMDB missing poster, switching to IMDB")
        ...
        if LANDSCAPE_POSTER and TMDB_POSTER:
            poster_url = details.get("backdrop_url", poster_url)
        ...
        movie_doc = {
            ...
            "imdb_url": details.get("url", "") if not TMDB_POSTER or not details.get("tmdb_url") else details.get("tmdb_url"),
...
async def send_movie_update(bot, base_name):
    ...
    for attempt in range(max_retries):
        try:
            ...
            [
                InlineKeyboardButton('♻️ How to Download ♻️', url="https://t.me/+dVRLYHXJztJlMmY9"
                )
            ]])
            ...
            if poster_url and not LINK_PREVIEW:
                try:
                    size = (2560, 1440) if LANDSCAPE_POSTER and TMDB_POSTER else (853, 1280)
